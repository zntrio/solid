...
var pasetoPrivateKey = []byte(`{"kid":"foo", "kty": "OKP","d": "XlEceI8Qol54ytI0Uxe7GqAGl6_nief996QthhCnpew","use": "sig","crv": "Ed25519","x": "g1nc4Wa7fWcrRuGqiFOb8MlikE-EYHgWinC7QqGBmzU", "alg": "EdDSA"}`)
...
func Test_accessTokenGenerator_Generate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				keyProvider: func(_ context.Context) (*jose.JSONWebKey, error) {
					...
					// Decode JWK
					err := json.Unmarshal(pasetoPrivateKey, &privateKey)
					if err != nil {
		...
		{
			name: "ed25519 sign with confirmation",
			fields: fields{
				keyProvider: func(_ context.Context) (*jose.JSONWebKey, error) {
					var privateKey jose.JSONWebKey
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			c := AccessToken(tt.fields.keyProvider)
