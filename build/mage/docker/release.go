...
FROM zntrio/solid-tools as compiler
...
LABEL \
    ...
    org.label-schema.vendor="zntr.io" \
...
func Release(cmd *artifact.Command) func() error {
	return func() error {
		...
		relVer, err := semver.NewVersion(release)
		if err != nil {
		...
		c := exec.Command("docker", "build",
			"-t", fmt.Sprintf("zntrio/%s:artifacts-%s", cmd.Kebab(), relVer.String()),
			"-f", "-",
			...
			"--cache-from", "zntrio/solid-tools",
			"--cache-from", fmt.Sprintf("zntrio/%s:artifacts-%s", cmd.Kebab(), relVer.String()),
			".",
		)
