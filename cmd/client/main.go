...
func callback(solidClient client.Client, config *session.Config, prover dpop.Prover, jarDecoder jarm.ResponseDecoder) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var (
			ctx         = r.Context()
			responseRaw = r.FormValue("response")
		)
		...
		if response.Error != nil {
			withError(w, r, http.StatusBadRequest, response.Error)
			return
		}
...
func dpopProver(keyProvider jwk.KeyProviderFunc) dpop.Prover {
	// Signer options
	dpopSigner := jwt.DPoPSigner(jose.ES384, keyProvider)
...
func arEncoder(keyProvider jwk.KeyProviderFunc) (jwsreq.AuthorizationEncoder, error) {
	arSigner := jwt.RequestSigner(jose.ES384, keyProvider)
